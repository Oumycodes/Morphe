...
type ScanState = 'ready' | 'front_positioning' | 'front_detected' | 'side_intro' | 'side_positioning' | 'side_detected' | 'analyzing' | 'result'
...
export default function ScanScreen({ onBack, onResult }: { onBack: () => void, onResult?: (data: any) => void }) {
  ...
  useEffect(() => {
    ...
    if (scanState === 'side_positioning') {
      detectionTimer.current = setTimeout(() => {
        ...
        setTimeout(() => captureSideAndAnalyze(), 3000)
      }, 4000)
    }
    return () => { if (detectionTimer.current) clearTimeout(detectionTimer.current) }
  }, [scanState])
  ...
  const captureFront = async () => {
    ...
    try {
      ...
      console.log('FRONT CAPTURE ERROR:', e?.message || e)
      setScanState('ready')
    }
  }
  ...
  const captureSideAndAnalyze = async () => {
    if (!camera.current || !frontUriRef.current) {
      console.log('SIDE ERROR: no camera or no frontUri', { camera: !!camera.current, frontUri: frontUriRef.current })
      return
    }
    ...
    try {
      ...
      if (!sidePhoto?.uri) throw new Error('No side photo')

      console.log('Sending to backend:', { front: frontUriRef.current, side: sidePhoto.uri })
      ...
      console.log('SIDE ANALYZE ERROR:', msg)
      setErrorMsg(msg)
      ...
      <View style={s.container}>
        ...
        <View style={s.resultWrap}>
          ...
          <Text style={s.resultSub}>
            Cycle {result.cycleNumber} · {scoreData?.pts_change > 0 ? '+' : ''}{scoreData?.pts_change || 0} pts
          </Text>
          <View style={s.measureCard}>
            ...
            {[
              ...
              { label: 'Glutes', val: m.glute_projection_ratio, color: '#a78bfa', pct: Math.min((m.glute_projection_ratio || 0) * 200, 100) },
      ...
      <View style={s.container}>
        ...
        <View style={s.analyzeWrap}>
          ...
          <View style={s.stepsWrap}>
            ...
            <Text style={s.holdStill}>Analyzing front + side scans...</Text>
          </View>
        </View>
      </View>
    )
      ...
      <View style={s.container}>
        ...
        <View style={s.introWrap}>
          ...
          <Text style={s.introSub}>
            Turn 90° to your right so your side profile faces the camera. This lets Morphe measure your glute projection and waist depth.
          </Text>
          ...
          <TouchableOpacity
            style={s.startBtn}
            onPress={() => setScanState('side_positioning')}
          >
      ...
      <View style={s.bottomBar}>
        {scanState === 'ready' && (
          <>
            ...
            <TouchableOpacity
              style={s.startBtn}
              onPress={() => setScanState('front_positioning')}
            >
              <Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
        ...
        {isPositioning && (
          <Text style={s.instructText}>
            {detected
              ...
              : isSide
              ? 'Turn 90° · full body in frame · hold still'
...
const s = StyleSheet.create({
  ...
  introWrap: { flex: 1, padding: 32, paddingTop: 80, justifyContent: 'center' },
  ...
  introSub: { fontSize: 14, color: '#4b7ab5', lineHeight: 22, marginBottom: 32 },
  ...
  morpheTopLabel: { fontSize: 10, fontWeight: '700', letterSpacing: 1.2, color: '#22d3ee', marginBottom: 24 },
  ...
  cameraCircle: { width: 184, height: 184, borderRadius: 92, overflow: 'hidden', borderWidth: 1.5, borderColor: '#0d1f35' },
  scanLine: {
    position: 'absolute', left: 0, right: 0, height: 1.5, backgroundColor: '#22d3ee',
    shadowColor: '#22d3ee', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 6,
  },
  gridLine: { position: 'absolute', left: 0, right: 0, height: 0.5, backgroundColor: '#22d3ee18' },
  ...
  resultBtnSec: { backgroundColor: '#0d1f35', borderRadius: 12, padding: 14, alignItems: 'center' },
